...
// DT: Dont hide newline, our Lexer handle that// New_line : '\n' -> channel(HIDDEN);
New_line : '\n' ;
...
// DT: Dont hide newline, our Lexer handle that//WHITE_SPACE : [ \t\r\n]+ -> channel(HIDDEN) ;
WHITE_SPACE : [ \t\r\n]+ ;
...
//binary_operator  : '+' | '-' | '*' | '/' | '%' | '==' | '!=' | '===' | '!==' | '&&' | '||' | '**' | '<' | '<=' | '>' | '>=' | '&' | '|' | '^' | '^~' | '~^' | '>>' | '<<' | '>>>' | '<<<' ;
// DT: change '<'/'>' to token
binary_operator  : '+' | '-' | '*' | '/' | '%' | '==' | '!=' | '===' | '!==' | '&&' | '||' | '**' | Left_angle | '<=' | Right_angle | '>=' | '&' | '|' | '^' | '^~' | '~^' | '>>' | '<<' | '>>>' | '<<<' ;
...
Define_comma: ',' ;
Define_equal: '=' ;
Define_indetifier: [a-zA-Z_] ( [a-zA-Z_0-9] )* ;
Define_any_char: '\\' '\r'? '\n' | ~[\r\n] ;
Define_new_line: '\r'? '\n' -> popMode ;
